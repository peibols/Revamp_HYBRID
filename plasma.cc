...
template <size_t T, size_t X, size_t Y>
static inline double trilinear(const double (&data)[T][X][Y], int it, int ix, int iy, double dt, double dx, double dy) {
    double v = 0.0;
    v += data[it][ix][iy] * (1. - dt) * (1. - dx) * (1. - dy);
    v += data[it + 1][ix][iy] * dt * (1. - dx) * (1. - dy);
    v += data[it][ix + 1][iy] * (1. - dt) * dx * (1. - dy);
    v += data[it][ix][iy + 1] * (1. - dt) * (1. - dx) * dy;
    v += data[it + 1][ix + 1][iy] * dt * dx * (1. - dy);
    v += data[it][ix + 1][iy + 1] * (1. - dt) * dx * dy;
    v += data[it + 1][ix][iy + 1] * dt * (1. - dx) * dy;
    v += data[it + 1][ix + 1][iy + 1] * dt * dx * dy;
    return v;
}
...
*/
		...
		for (int cr=0; cr<k; cr++) //loop over the final particles
		{
			...
			//cout << "lambda= " << Q[fam[0]][3]/P[fam[0]][3] << " id= " << int(P[fam[0]][11]) << " ini= " << P[fam[0]][7] << endl;

		}
//----------------------------------------------------------------------------

                cout << endl;
		if (k!=0) check << "NEXT" << " " << " 0. " << " " << " 0. " << " " << xcre << " " << ycre << " " << b << " " << pthat << " " << count << " " << cross_err << " " << cross << " " << weight << "\n";
...
double getGrid(double tau, double x, double y, double *dt, double *dx, double *dy, int *it, int *ix, int *iy) {
    // Time index (floor) and normalized fractional position
    double tof = (tau - tau0) / deltat;
    *it = static_cast<int>(std::floor(tof));
    *it = std::clamp(*it, 0, std::max(0, maxt - 2));
    *dt = tof - static_cast<double>(*it);

    // Spatial indices: assume grid runs from -maxx/2..maxx/2 (or equivalent)
    double xgrid = x / deltax + double(maxx) / 2.0;
    *ix = static_cast<int>(std::floor(xgrid));
    *ix = std::clamp(*ix, 0, std::max(0, maxx - 2));
    *dx = xgrid - static_cast<double>(*ix);

    double ygrid = y / deltay + double(maxy) / 2.0;
    *iy = static_cast<int>(std::floor(ygrid));
    *iy = std::clamp(*iy, 0, std::max(0, maxy - 2));
    *dy = ygrid - static_cast<double>(*iy);
...
// temp in fluid rest frame
	...
	gete = trilinear(hydrot, it, ix, iy, dt, dx, dy);
	return gete * 0.2;
}
...
double gE(double tau, double x, double y) {
	...
	gener = trilinear(hydroe, it, ix, iy, dt, dx, dy);
	return gener;
}
...
double gVx(double tau, double x, double y) {
	...
	gvelx = trilinear(hydrox, it, ix, iy, dt, dx, dy);
...
double gVy(double tau, double x, double y) {
	...
	gvely = trilinear(hydroy, it, ix, iy, dt, dx, dy);
