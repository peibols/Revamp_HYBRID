...
void do_wake(vector<Quench> quenched, vector<Parton> partons, vector<Wake> &wake, numrand &nr)
{
  ...
  for (unsigned int i=0; i<quenched.size(); i++)
  {
    ...
    double scaler = abs(delta[3])/sqrt(pow(delta[0],2.)+pow(delta[1],2.)+pow(delta[2],2.));
    delta[0] *= scaler;
    delta[1] *= scaler;
    delta[2] *= scaler;
    // cout << " delta[3]= " << delta[3] << " delta space " << sqrt(pow(delta[0],2.)+pow(delta[1],2.)+pow(delta[2],2.)) << endl;
    ...
    if (mtlost>transcut)
    {
      ...
      } while(momback[3]*momback[3]<delta[3]*delta[3]);
...
void one_body(vector<Wake> &wake, vector<double> delta, vector<double> momback, double ptlost, double mtlost, double raplost, numrand &nr, int spe, int mode)
{
  ...
  double phirand, mtrand, cosphidif, rapdif;
  double r2, Temp;
  if (delta[3]<0.) {
    raplost *= -1;
    delta[0] *= -1;
  ...
  do {
    ...
    // 1. Pull max constants out
    constexpr double TWO_PI = 6.283185307179586476925286766559;
    constexpr double EPS_PT = 1e-6;
    ...
    // 3. Cheap transforms
    phirand = TWO_PI * u2;    // uniform angle
    r2      = maxptsq * u1;   // uniform in r^2
    raprand = maxrap * (2*u3 - 1.0);

    // 4. Only convert radius once accepted (or take sqrt immediately if you must)
    ptrand  = std::sqrt(r2 + EPS_PT);

    // 5. sincos in one shot
    double s, c;
    sincos(phirand, &s, &c);
    pxrand = ptrand * c;
    pyrand = ptrand * s;
    // mtrand=sqrt(ptrand*ptrand+masstra[spe]*masstra[spe]);
    cosphidif=(delta[0]*pxrand+delta[1]*pyrand)/(ptlost*ptrand);
    //rapdif=raplost-raprand;
    rapdif=raprand;
    
    Temp=thermal(spe,ptrand);

    mtrand=sqrt(r2+masstra[spe]*masstra[spe]);

    //Usual expression
    cooper=exp(-mtrand/Temp*cosh(rapdif))*mtrand/pow(Temp,5.)*cosh(rapdif)*
    (ptrand*3.*ptlost/mtlost*cosphidif+mtrand*cosh(rapdif))/normcoop[spe];

    if (cooper!=cooper) { 
      cout << " Cooper NaN \n", cooper=0.; cout << " mtrand= " << mtrand << " Temp= " << Temp << " rapdif= " << rapdif << " phidif= " << cosphidif << " ptrand= " << ptrand << endl; 
