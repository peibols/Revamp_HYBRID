...
int main (int argc, char** argv)
{
	...
	double P[1000][14], num, px, py, pz, E, q, mom, qge, ini, col, acol, ide;
	double x,y,z,b;
	...
	do
	{
		...
		while (infile1 >> snum >> px >> py >> pz >> E >> q >> mom >> qge >> ini >> col >> acol >> ide)
		{
			if (snum!="NEXT")
			{
				...
				P[int(num)][10]=acol;
				P[int(num)][11]=ide;
				P[int(num)][13]=mom;    //Original mom, for analysis purposes
				//Remnants
			...
			else
			{
                ...
                int nhard = 0;
		//Assign primary branch status, starting from two hard
		for (int ip=0; ip<1000; ip++) {
                  if (P[ip][7]==0 || P[ip][5]!=3) continue;
...
*/
                for (int ip=0; ip<1000; ip++) {
                  if (P[ip][3]==0.) continue;
		  cout << " particle= " << ip << " isdone= " << P[ip][8] << " and mom= " << P[ip][13] << " and effective mom= " << P[ip][5] << " is primary= " << P[ip][12] << endl;
		}
		...
		for (int cr=0; cr<k; cr++) //loop over the final particles
		{
			...
			for (unsigned a=0; a<50; a++)
			{
				...
				if (P[mom][8]==0.) //if the mom is not done, then add to the family tree
				{
					...
					fam[fr]=mom;
					cout <<  " Adding particle= " << mom << " to the list " << endl;
					ind=mom;
				}
			...
			for (int w=fr; w>-1; w--)
			{
				...
				{
				        cout << " Doing particle = " << fam[w] << endl;
					dE=gdE(we,P[fam[w]][6],acum-previous,previous,&x,&y,&z,&pexe,&peye,&peze);
					//Check if resolved
					int tmom = P[fam[w]][13];
					if (tmom!=3) {
					  double zval = P[fam[w]][3]/P[tmom][3];
					  //Look for sister
					  int mysis = -1000;
					  for (unsigned int im=0; im<1000; im++) {
					    if (im == fam[w]) continue;
					    if (P[im][3]==0.) continue;
                                            int smom = P[im][13];
					    if (tmom == smom) { mysis = im; break; }
					  }
					  if (mysis == -1000) {
					    cout << " Could not find sis, exiting!" << endl;
					    exit(1);
					  }
					  //z cross check
					  double onemzval = P[mysis][3]/P[tmom][3];
					  bool zcheckok=1;
					  if (fabs(onemzval+zval)-1.>0.01) {
                                            //cout << " Problem with z+1-z != 1, it is = " << onemzval+zval << endl;
					    //cout << " mom energy is= " << P[tmom][3] << " and index= " << tmom << endl;
					    //exit(1);
					    zcheckok=0;
					  }
					  else {
					  ...
					  //double mymomentum = sqrt(pow(P[fam[w]][0],2.)+pow(P[fam[w]][1],2.)+pow(P[fam[w]][2],2.));
					  //double sismomentum = sqrt(pow(P[mysis][0],2.)+pow(P[mysis][1],2.)+pow(P[mysis][2],2.));
					  double mymomentum = sqrt(pow(P[fam[w]][0],2.)+pow(P[fam[w]][1],2.));
					  double sismomentum = sqrt(pow(P[mysis][0],2.)+pow(P[mysis][1],2.));
					  //cout << " mymomentum= " << mymomentum << " sismomentum= " << sismomentum << endl;
					  double phidif = acos((P[fam[w]][0]*P[mysis][0]+P[fam[w]][1]*P[mysis][1])/mymomentum/sismomentum);
					  if (phidif!=phidif) phidif=0.;
					  double myrap = 1/2.*log((P[fam[w]][3]+P[fam[w]][2])/(P[fam[w]][3]-P[fam[w]][2]));
					  double sisrap = 1/2.*log((P[mysis][3]+P[mysis][2])/(P[mysis][3]-P[mysis][2]));
					  double angleval = sqrt(phidif*phidif+pow(myrap-sisrap,2.));
					  //double ktval = zval*onemzval*P[tmom][3]*fabs(sin(angleval));
					  double ktval = min(mymomentum,sismomentum)*angleval;
					  double zadam = min(mymomentum,sismomentum)/(mymomentum+sismomentum);
                                          if (mymomentum < sismomentum) lundfile << weight << " " << ktval << " " << zadam << " " << angleval << " " << dE << " " << dE/P[fam[w]][3] << " " << endl;
					  if (P[tmom][12]==1) {
                                            cout << " Primary participant = " << fam[w] << " with mom= " << tmom << endl;
					  }
                                          if (mymomentum < sismomentum && P[tmom][12]==1) plundfile << weight << " " << ktval << " " << zadam << " " << angleval << " " << dE << " " << dE/P[fam[w]][3] << " " << endl;
					}
					///////////
					Q[fam[w]][0]=pexe;
                    			Q[fam[w]][1]=peye;
...
{ //momx,momy, and momz are the finishing postions, or the starting postions of its daughters
	...
	ti=t0;
        ...
        if (rpower<0.00001) return 100000000000;
	...
	for (unsigned i=0; i<=100000000; i++)
	{
		...
		double propermom=sqrt(ti*ti-momz*momz);
                ...
                //DEBUG
	        propermom = max(propermom,tquench);
