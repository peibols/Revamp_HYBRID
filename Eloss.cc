...
void do_eloss(vector<Parton> partons, vector<Quench> &quenched, double xcre, double ycre, numrand &nr, double kappa, double alpha, int tmethod, int mode, int new_eloss);
...
void do_eloss(vector<Parton> partons, vector<Quench> &quenched, double xcre, double ycre, numrand &nr, double kappa, double alpha, double lambda, int tmethod, int mode, int new_eloss)
{
  //Tag final particles
  vector<int> FinId;
  ...
  for (unsigned int i = 0; i < FinId.size(); i++)
  {
    ...
    for (unsigned int w = Fam.size(); w>0; w--)
    {
      ...
      double mass=quenched[tp].GetMass();
      ...
      double fluidvx=quenched[tp].GetFluidVx();
      double fluidvy=quenched[tp].GetFluidVy();
      double fluidvz=quenched[tp].GetFluidVz();
      bool freezoutcrosser = false;

      //If colored particle
      if (abs(quenched[tp].GetId())<=6 || quenched[tp].GetId()==21)
      {
        loss_rate(p, pos, mass, tof, quenched[tp].GetId(), nr, kappa, alpha, lambda, tmethod, mode, length, tlength, new_eloss, fluidvx,fluidvy,fluidvz, freezoutcrosser);
        quenched[tp].SetFluidV(fluidvx,fluidvy,fluidvz);
        quenched[tp].SetFreezoutCrosser(freezoutcrosser);
      }
      else
      {
