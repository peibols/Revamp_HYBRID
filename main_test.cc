...
void do_lund_med(vector<Quench> partons, vector<Hadron> &hadrons, int hadro_type);
...
int main(int argc, char** argv)
{
  ...
  //}
    ...
    quenchandrecoil.clear();
    double n_px[5]={0.0703033,0.706187,9.35888,-0.735208,-0.479988};
    double n_py[5]={0.480759,-0.787433,15.3285,0.676806,0.0392289};
    double n_pz[5]={1.15171,0.666145,10.5105,73.3292,-36.3245};
    double n_mass[5]={1.5,1.5,0.,0.33,0.33};
    int n_id[5]={4,-4,22,2,-2};
    for (int inew=2; inew<3; inew++) {
      double entest=sqrt(pow(n_px[inew],2.)+pow(n_py[inew],2.)+pow(n_pz[inew],2.)+pow(n_mass[inew],2.));
      vector<double> ptest{n_px[inew],n_py[inew],n_pz[inew],entest};
      quenchandrecoil.push_back( Parton ( ptest, 1000000., 0, 0, -1, -1, n_id[inew], "ps", 0, 0, true ) );
    }

    do_lund_med(quenchandrecoil,qhadrons,hadro_type);
