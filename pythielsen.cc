...
do {
	...
	double cross = pythia.info.sigmaGen();
	double cross_err = pythia.info.sigmaErr();
	double pthat = pythia.info.pTHat();

	//cout << "cross_err= " << cross_err << " pthat= " << pthat << endl;

	geneal << "NEXT" << " 0. " << " 0. " << " 0. " << "0.00" << count << " 0. " << " " << weight << " " << cross << " " << cross_err << " " << pthat << " 0. " << " 0. " << "\n" ;
	count+=1;
} while (count<N);

geneal << "END" << " " << std::setprecision(9) << pythia.info.sigmaGen() << " " << pythia.info.weightSum() << endl;
geneal.close();
pythia.stat();
