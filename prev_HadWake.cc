...
void do_wake(vector<Quench> quenched, vector<Parton> partons, vector<Wake> &wake, numrand &nr)
{
  ...
  for (unsigned int i=0; i<quenched.size(); i++)
  {
    ...
    vector<double> delta;
    if (quenched[i].GetOrig()=="hole") continue;
    else if (quenched[i].GetOrig()=="recoiler") delta = quenched[i].orig_en()-quenched[i].vGetP();
    else delta = partons[i].vGetP()-quenched[i].vGetP();
    //cout << "Orig= " << quenched[i].GetOrig() << " delta= " << delta[3] << endl;
    if (delta[3]<=0.) continue;				//Swiftly skip guys who didn't lose energy

    delta[3]=sqrt(delta[0]*delta[0]+delta[1]*delta[1]+delta[2]*delta[2]);
    double sour_virt=delta[3]*delta[3]-delta[0]*delta[0]-delta[1]*delta[1]-delta[2]*delta[2];
    if (sour_virt<0. || sour_virt>0.) {
      //cout << " sour_virt= " << sour_virt << endl;
    }

    total_injected += delta[3];

    //Reorient
    vector<double> or_delta;
    or_delta.push_back(quenched[i].orient()[0]*delta[3]);
    or_delta.push_back(quenched[i].orient()[1]*delta[3]);
    or_delta.push_back(quenched[i].orient()[2]*delta[3]);
    or_delta.push_back(delta[3]);

    // check or_Delta
    for (unsigned a=0; a<3; a++) {
      cout << " delta a= " << a << " = " << delta[a] << " or_delta  = " << or_delta[a] << endl;
    }
    double mod_or=sqrt(quenched[i].orient()[0]*quenched[i].orient()[0]+quenched[i].orient()[1]*quenched[i].orient()[1]+quenched[i].orient()[2]*quenched[i].orient()[2]);
    cout << " mod or= " << mod_or << endl;

    //Set new delta accounting for deflection
    //delta = or_delta;
